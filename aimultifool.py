...
class AiMultiFoolApp(App, InferenceMixin, ActionsMixin, UIMixin, VectorMixin, RLMMixin):
    ...
    def populate_right_sidebar(self, filter_text="", highlight_item_name=None):
        ...
        # Pin "Analysis" and "Scene Tools" at the top, then sort the rest alphabetically
        pinned_categories = ["Analysis", "Scene Tools"]
        other_categories = sorted([cat for cat in grouped.keys() if cat not in pinned_categories])
        sorted_categories = [cat for cat in pinned_categories if cat in grouped.keys()] + other_categories
    ...
    async def on_list_view_selected(self, event: ListView.Selected) -> None:
        ...
        try:
            if event.list_view.id == "list-characters":
                ...
                # Immediately clear selection from all action lists to prevent highlight
                list_view = event.list_view
                for action_list in self.query(".action-list"):
                    action_list.index = None
                ...
                if ":::" in data_packed:
                    ...
                    if len(parts) == 3:
                        ...
                        # Ensure selection stays cleared (already cleared at start, but double-check)
                        for action_list in self.query(".action-list"):
                            action_list.index = None
                            for item in action_list.query("ListItem"):
                                item.remove_class("--highlight")
    ...
    async def chat_manager_callback(self, result):
        ...
        if action == "load":
            ...
            if messages:
                ...
                # Load messages (model settings are not restored - users can use whatever model they want)
