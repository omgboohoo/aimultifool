...
class RLMChatScreen(ModalScreen):
    """The modal for managing RLM (Recursive Language Models) chats."""
    def compose(self) -> ComposeResult:
        with Vertical(id="rlm-chat-dialog", classes="modal-dialog"):
            yield Label("RLM Chat (Recursive Language Models)", classes="dialog-title")
            
            yield Label("Manage your RLM context stores.", classes="dialog-subtitle")
            yield Label("RLM stores full conversation history externally and queries it recursively.", classes="dialog-subtitle")
            
            with Horizontal(id="rlm-input-container"):
                yield Input(placeholder="New RLM Chat Name", id="input-rlm-name")
                yield Button("Create", id="btn-rlm-create")
            
            yield Label("Password (Optional - for encryption):", classes="label")
            yield Input(placeholder="Password / Passphrase", id="input-rlm-password", password=False)
            
            yield Label("Existing RLM Chats:", classes="section-label")
            yield ListView(id="list-rlm-chats")
            
            with Horizontal(classes="buttons"):
                yield Button("Load", id="btn-rlm-load")
                yield Button("Inspect", id="btn-rlm-inspect")
                yield Button("Duplicate", id="btn-rlm-duplicate")
                yield Button("Rename", id="btn-rlm-rename")
            
            with Horizontal(classes="buttons"):
                yield Button("Delete", id="btn-rlm-delete")
                yield Button("Disable RLM Chat", id="btn-rlm-disable")
                yield Button("Close", id="btn-close")
    ...
    def on_mount(self) -> None:
        ...
        title.can_focus = True
        title.focus()
        self.update_rlm_list()
        # Disable the 'Disable' button if RLM chat is not currently enabled
        self.query_one("#btn-rlm-disable").disabled = not getattr(self.app, "enable_rlm_chat", False)
        self.update_button_states()

    def update_button_states(self):
        """Enable or disable management buttons based on selection."""
        try:
            list_view = self.query_one("#list-rlm-chats", ListView)
            has_selection = list_view.highlighted_child is not None
            
            for btn_id in ["btn-rlm-load", "btn-rlm-inspect", "btn-rlm-duplicate", "btn-rlm-rename", "btn-rlm-delete"]:
                self.query_one(f"#{btn_id}").disabled = not has_selection
        except Exception:
            pass

    def on_list_view_highlighted(self, event: ListView.Highlighted) -> None:
        if event.list_view.id == "#list-rlm-chats" or event.list_view.id == "list-rlm-chats":
            self.update_button_states()

    def update_rlm_list(self):
        rlm_dir = Path(__file__).parent / "rlmcontexts"
        if not rlm_dir.exists():
            rlm_dir.mkdir(parents=True, exist_ok=True)
            
        chats = [d.name for d in rlm_dir.iterdir() if d.is_dir()]
        list_view = self.query_one("#list-rlm-chats", ListView)
        list_view.clear()
        for chat in sorted(chats):
            is_encrypted = (rlm_dir / chat / ".encrypted").exists()
            display_name = f"🔒 {chat}" if is_encrypted else chat
            item = ListItem(Label(display_name))
            item.chat_name = chat  # Custom attribute for reliability
            item.is_encrypted = is_encrypted
    ...
    async def on_button_pressed(self, event: Button.Pressed) -> None:
        if event.button.id == "btn-rlm-create":
            name = self.query_one("#input-rlm-name", Input).value.strip()
            password = self.query_one("#input-rlm-password", Input).value
            if name:
                self.dismiss({"action": "create", "name": name, "password": password})
            else:
                self.app.notify("Please enter a name for the new RLM chat.", severity="error")
        elif event.button.id == "btn-rlm-load":
            try:
                selected = self.query_one("#list-rlm-chats", ListView).highlighted_child
                if selected and hasattr(selected, "chat_name"):
                    password = self.query_one("#input-rlm-password", Input).value
                    if selected.is_encrypted and not password:
                        self.app.notify("This RLM chat is encrypted! Please enter the password below.", severity="warning")
                        self.query_one("#input-rlm-password").focus()
                        return
                    
                    # Validate password before dismissing modal
                    if selected.is_encrypted:
                        try:
                            self.app.check_rlm_password(selected.chat_name, password)
                        except Exception as e:
                            self.app.notify(str(e), severity="error")
                            self.query_one("#input-rlm-password").value = ""
                            self.query_one("#input-rlm-password").focus()
                            return

                    self.dismiss({"action": "load", "name": selected.chat_name, "password": password})
                else:
                    self.app.notify("No RLM chat selected.", severity="warning")
            except Exception as e:
                self.app.notify(f"Load error: {e}", severity="error")
        elif event.button.id == "btn-rlm-inspect":
            try:
                item = self.query_one("#list-rlm-chats", ListView).highlighted_child
                if item and hasattr(item, "chat_name"):
                    password = self.query_one("#input-rlm-password", Input).value
                    if item.is_encrypted and not password:
                         self.app.notify("This RLM chat is encrypted. Provide a password to see plaintext.", severity="information")
                    self.app.push_screen(RLMInspectScreen(item.chat_name, password=password), lambda _: self.query_one(".dialog-title").focus())
                else:
                    self.app.notify("No RLM chat selected.", severity="warning")
            except Exception as e:
                self.app.notify(f"Inspect error: {e}", severity="error")
        elif event.button.id == "btn-rlm-duplicate":
            item = self.query_one("#list-rlm-chats", ListView).highlighted_child
            if item and hasattr(item, "chat_name"):
                def on_duplicate(new_name):
                    if new_name:
                        import shutil
                        old_name = item.chat_name
                        old_path = Path(__file__).parent / "rlmcontexts" / old_name
                        new_path = Path(__file__).parent / "rlmcontexts" / new_name
                        if new_path.exists():
                            self.app.notify("An RLM chat with that name already exists.", severity="error")
                        else:
                            try:
                                shutil.copytree(old_path, new_path, ignore=shutil.ignore_patterns('.lock', 'LOCK', '*.lock'))
                                self.app.notify(f"Duplicated {old_name} to {new_name}")
                ...
                self.app.notify("No RLM chat selected.", severity="warning")
        elif event.button.id == "btn-rlm-rename":
            item = self.query_one("#list-rlm-chats", ListView).highlighted_child
            if item and hasattr(item, "chat_name"):
                if getattr(self.app, "rlm_chat_name", None) == item.chat_name:
                    self.app.notify("Cannot rename the currently active RLM chat.", severity="error")
                    return
                def on_rename(new_name):
                    if new_name and new_name != item.chat_name:
                        import shutil
                ...
                self.app.notify("No RLM chat selected.", severity="warning")
        elif event.button.id == "btn-rlm-delete":
            try:
                item = self.query_one("#list-rlm-chats", ListView).highlighted_child
                if item and hasattr(item, "chat_name"):
                    chat_name = item.chat_name
                    
                    # If this is the active RLM chat, we must stop/close it first
                    if getattr(self.app, "rlm_chat_name", None) == chat_name:
                        if hasattr(self.app, "close_rlm_context"):
                            self.app.close_rlm_context()
                        self.app.enable_rlm_chat = False
                        self.app.rlm_chat_name = None
                        self.app.notify(f"Deactivating '{chat_name}' before deletion...")

                    import shutil
                    rlm_dir = Path(__file__).parent / "rlmcontexts" / chat_name
                    if rlm_dir.exists():
                        try:
                            shutil.rmtree(rlm_dir)
                            self.app.notify(f"Deleted RLM chat: {chat_name}", severity="warning")
                            self.update_rlm_list()
                        except Exception as e:
                            self.app.notify(f"Delete failed: {e}. Is the context store still in use?", severity="error")
                else:
                    self.app.notify("No RLM chat selected.", severity="warning")
            except Exception as e:
                self.app.notify(f"Delete error: {e}", severity="error")
        elif event.button.id == "btn-rlm-disable":
            if hasattr(self.app, "action_disable_rlm_chat"):
                await self.app.action_disable_rlm_chat()
                event.button.disabled = True
            else:
                self.app.notify("Disable action not found.", severity="error")
        elif event.button.id == "btn-close":
        ...
        # Reset focus to title for buttons that don't dismiss
        if event.button.id not in ["btn-close", "btn-rlm-create", "btn-rlm-load"]:
            try:
                self.query_one(".dialog-title").focus()
            except Exception:
                pass
...
class RLMInspectScreen(ModalScreen):
    """Screen for inspecting RLM context stores."""
    def __init__(self, chat_name: str, password: str = None):
        super().__init__()
        self.chat_name = chat_name
        self.password = password
    ...
    def compose(self) -> ComposeResult:
        with Container(id="rlm-inspect-dialog", classes="modal-dialog"):
            yield Label(f"Inspect RLM Context: {self.chat_name}", classes="dialog-title")
            yield TextArea("Loading...", id="rlm-content", read_only=True)
            with Horizontal(classes="buttons"):
                yield Button("Close", id="btn-close", variant="default")

    def on_mount(self) -> None:
        self.load_rlm_context()

    @work(exclusive=True, thread=True)
    def load_rlm_context(self):
        try:
            rlm_dir = Path(__file__).parent / "rlmcontexts" / self.chat_name
            if not rlm_dir.exists():
                self.app.call_from_thread(self.update_content, "RLM context store not found!")
                return
            ...
            if context_file.exists():
                try:
                    ...
                    # Decrypt if needed
                    if self.password and data.get("encrypted", False):
                        from utils import decrypt_data
                        ...
                        for msg in encrypted_messages:
                            try:
                                decrypted.append({
                                    "role": msg.get("role"),
                                    "content": decrypt_data(msg.get("content", ""), self.password)
                                })
                            except Exception:
                                decrypted.append({"role": msg.get("role"), "content": "[Decryption failed]"})
                        data["messages"] = decrypted
                    
                    # Format for display
                    content = f"RLM Context Store: {self.chat_name}\n"
                    content += f"Total Messages: {len(data.get('messages', []))}\n"
                    content += f"Encrypted: {data.get('encrypted', False)}\n\n"
                    content += "Messages:\n" + "="*50 + "\n\n"
                    
                    for i, msg in enumerate(data.get("messages", [])[:100]):  # Show first 100
                        role = msg.get("role", "unknown")
                        msg_content = msg.get("content", "")[:500]  # Truncate long messages
                        content += f"[{i}] {role.upper()}: {msg_content}\n\n"
                    
                    if len(data.get("messages", [])) > 100:
                        content += f"\n... and {len(data.get('messages', [])) - 100} more messages"
                    
                    self.app.call_from_thread(self.update_content, content)
                except Exception as e:
                    self.app.call_from_thread(self.update_content, f"Failed to load context: {e}")
            else:
                self.app.call_from_thread(self.update_content, "Context file not found. This RLM chat may be empty.")
        except Exception as e:
            self.app.call_from_thread(self.update_content, f"Error: {e}")

    def update_content(self, content: str):
        try:
            widget = self.query_one("#rlm-content", TextArea)
            widget.load_text(content)
        except Exception:
            pass
